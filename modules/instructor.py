    ...
    if not st.session_state.instructor_authenticated:
        ...
        with col2:
            if st.button("🔓 Unlock", type="primary", use_container_width=True):
                # Supplied by the environment. Previously a literal that
                # was also printed on the page below it, which made the
                # protection decorative.
                import hmac
                import os

                expected = os.environ.get("NANOBIO_INSTRUCTOR_PASSWORD", "")
                if expected.strip() and hmac.compare_digest(password, expected):
        ...
        st.markdown("---")
        st.info("**Note for Instructors**: the password is read from the NANOBIO_INSTRUCTOR_PASSWORD environment variable. There is no built-in default, and the value is not shown here.")
