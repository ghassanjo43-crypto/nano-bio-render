...
# DEFECT-D11 FIX (Phase 2 Step 1)
# -------------------------------
# The module-level `init_db()` call was removed (it appeared TWICE in this file,
# so importing auth.py initialised the database twice). Import-time database
# writes make the module untestable without filesystem side effects and are
# incompatible with Render's ephemeral filesystem.
#
# Initialisation is now explicit and idempotent:
#   * every public function calls `_ensure_initialized()` lazily, so existing
#     callers see no behavioural change;
#   * `initialize_database()` is the explicit startup entry point, called from
#     Login.py and intended to become a FastAPI lifespan / Alembic step.
#
# Previously: init_db()
...
def _ensure_initialized() -> None:
    """Create the schema once, on first use rather than on import."""
...
def initialize_database() -> None:
    """Explicit startup initialisation. Idempotent.

    Call this from application startup (Streamlit entry point today, FastAPI
    lifespan after the migration) instead of relying on import side effects.
    """
    _ensure_initialized()


def reset_initialization_state() -> None:
    """Forget that initialisation happened. For tests that repoint ``DB_PATH``."""
    global _DB_INITIALIZED
    _DB_INITIALIZED = False
...
# ---------------------------------
# The import-time call to _reset_admin_session() was REMOVED. Its own docstring
# said "Remove in production": every `import auth` silently rewrote the admin
# account's session_start and last_activity, which defeats the 30-minute
# inactivity timeout for that account and corrupts the audit trail.
#
# The function is intentionally left defined but uncalled, so that any developer
# workflow that genuinely needs it can call it explicitly. Do not re-add a
# module-level invocation. Database and session initialisation must become an
# explicit startup step in the FastAPI migration (see DEFECT-D11).
#
# Previously: _reset_admin_session()
...
# DEFECT-D11 FIX (Phase 2 Step 1): this was the SECOND module-level init_db()
# call in this file (the first is near the top). Importing auth.py therefore
# initialised the database twice. Both module-level calls are now removed; see
# `_ensure_initialized()` and `initialize_database()` above.
#
# Previously: init_db()
