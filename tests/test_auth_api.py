...
class TestRoleBasedAccess:
    ...
    def admin_only_route(self, app_and_client):
        """Mount a temporary admin-only route, and take it down again.

        The route used to be mounted inline by the first test that needed it
        and never removed. ``app`` is the real application object shared by
        the whole session, so the route survived into every later test — and
        the structural coverage guard in ``test_organization_routes.py``
        correctly reported it as an endpoint outside the organization
        boundary. It was a test artifact rather than a real hole, but a
        fixture that mutates shared state and does not clean up will
        eventually cause a failure that is genuinely confusing.
        """
        ...
        app.router.routes = [
            ...
            if getattr(r, "path", None) != "/api/v1/_test/admin-only"]

    def test_require_role_dependency_blocks_wrong_role(self, admin_only_route,
                                                       seeded_users):
        """Mount a temporary admin-only route and check enforcement."""
        _app, client = admin_only_route
    ...
    def test_admin_only_route_requires_authentication(self, admin_only_route):
        _app, client = admin_only_route
...
class TestPasswordHashing:
    ...
    def test_hash_is_not_the_plaintext(self):
        """Argon2id now, having been bcrypt.

        The assertion changed with the algorithm, deliberately: a test pinned
        to `$2` would have kept passing only for as long as nothing improved,
        and would have had to be deleted to allow the migration rather than
    ...
    def test_an_existing_bcrypt_password_still_verifies(self):
        """Nobody is locked out by the migration.

        This is the property that makes rehash-on-login possible at all: if a
        bcrypt hash stopped verifying the moment Argon2id arrived, every user
        would be locked out at once and an operator would be strongly tempted
        to hand out passwords to get them working again.
        """
        import bcrypt

        from nanobio_studio.app.core.security import verify_password

        stored = bcrypt.hashpw(b"an-existing-password", bcrypt.gensalt(rounds=4))
        assert verify_password("an-existing-password", stored.decode())
        assert not verify_password("wrong", stored.decode())

    def test_a_bcrypt_hash_is_marked_for_rehash_and_argon2_is_not(self):
        import bcrypt

        from nanobio_studio.app.core.security import hash_password, needs_rehash

        legacy = bcrypt.hashpw(b"x" * 20, bcrypt.gensalt(rounds=4)).decode()
        assert needs_rehash(legacy) is True
        assert needs_rehash(hash_password("a-fresh-strong-passphrase")) is False

    def test_there_is_exactly_one_password_implementation(self):
        """`core.security` re-exports; it does not implement.

        Two answers to "is this the right password" is one more than a system
        can safely have. This asserts the consolidation held: both modules
        expose the *same function object*, so a call site cannot get a second
        opinion by choosing a different import.
        """
        from nanobio_studio.app.core import passwords, security
