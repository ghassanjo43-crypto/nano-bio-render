...
def _client(tmp_path_factory):
    ...
    async def _seed():
        """Both accounts belong to one organization, as researchers.

        Report routes are organization-scoped now, so an account with no
        membership reaches nothing — correct, and it would make every test in
        this file fail for the wrong reason.

        Both are ``RESEARCHER`` with the default assigned-studies scope, which
        keeps ``test_another_users_assessment_is_not_reachable`` honest: the
        viewer cannot see the admin's assessment because their membership
        reaches no study it hangs off, not merely because a filter said so.

        The platform roles are unchanged. ``rpt_admin`` is a platform
        administrator *and* an organization researcher, which is the ordinary
        combination — the two ladders are independent, and retention purge is
        the one report action a platform administrator holds.
        """
        async with factory() as session:
            ...
            organization = Organization(
                slug="report-lab", name="Report Lab",
                status=OrganizationStatus.ACTIVE)
            session.add(organization)
            await session.flush()

            for name in (ADMIN_USER, VIEWER_USER):
                session.add(OrganizationMembership(
                    organization_id=organization.id, user_id=users[name].id,
...
class TestLifecycle:
    ...
    def test_download_is_an_attachment_and_not_sniffable(self):
        ...
        # `no-store` is asserted as PRESENT rather than as the whole value.
        # The security middleware now adds `no-cache, must-revalidate,
        # private` alongside it, which is strictly stronger — an equality
        # assertion would have had to be deleted to allow the improvement
        # rather than updated to describe it.
