...
Includes persistent session storage and idle timeout (30 minutes).
"""
...
def create_persistent_session(username: str, user_id: str, email: str, roles: list) -> str:
    ...
    # Store session with timestamps (UTC aware)
    sessions[token] = {
        "username": username,
        "user_id": user_id,
        "email": email,
...
def update_last_activity(token: str):
    """Update last activity timestamp for a session (UTC aware)"""
    sessions = load_persistent_sessions()
    
    if token in sessions:
        sessions[token]["last_activity"] = datetime.now(timezone.utc).isoformat()
        save_persistent_sessions(sessions)
...
class StreamlitAuth:
    ...
    def require_login_with_persistence(page_name: str = "This page") -> bool:
        ...
        # SECURITY CONTAINMENT (2026-07-30): URL-supplied session tokens are no
        # longer accepted. Honouring `?session_token=...` allowed anyone to
        # authenticate by supplying a token, and tokens are predictable
        # (`token_{username}_{unix_seconds}`). Stale values are cleared, not used.
...
def switch_page_with_token(page_path: str):
    """
    Switch to another page. The session token is NOT placed in the URL.

    SECURITY CONTAINMENT (2026-07-30)
    ---------------------------------
    This function previously wrote the session token into ``st.query_params``,
    putting a bearer credential in the URL where it leaks through browser
    history, proxy and server logs, and Referer headers. That write has been
    removed.

    The name is kept so every existing call site keeps working unchanged.
    ``st.session_state`` already carries the token across ``st.switch_page``
    within a browser session, so in-app navigation is unaffected. The only
    behaviour lost is silent re-authentication after a hard browser refresh,
    which now correctly requires logging in again.
