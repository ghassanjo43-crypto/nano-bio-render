...
def render_activity_log_viewer():
    ...
    if filter_type == "All":
        with col2:
            limit = st.number_input("Show entries", min_value=10, max_value=500, value=100, step=10)
...
def render_security_report():
    ...
    with col1:
        report_days = st.number_input("Days to analyze", min_value=1, max_value=90, value=7, step=1)
...
def render_compliance_report():
    ...
    with col1:
        report_days = st.number_input("Report period (days)", min_value=1, max_value=365, value=30, step=1, key="compliance_days")
