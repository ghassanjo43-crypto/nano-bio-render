...
# ---------------------------------------------------------------------------
# Explicit, idempotent initialisation (DEFECT-D11)
# ---------------------------------------------------------------------------
...
def _ensure_initialized() -> None:
    ...
    Idempotent and cheap after the first call. This replaces the module-level
    ``init_design_db()`` invocation so that ``import design_persistence`` has no
...
# -------------------------------
# The module-level `init_design_db()` call was REMOVED. Importing this module
# wrote a ~36 KB nano_bio.db into the current working directory before any
# configuration or path override could apply, which made the module untestable
# without filesystem side effects and is incompatible with Render's ephemeral
# filesystem. It actually leaked a database into the repository root during
# harness development.
#
# Initialisation is now explicit and idempotent. Every public function in this
# module calls `_ensure_initialized()` first, so callers see no behavioural
# change; the difference is that the write happens on first *use*, not on import.
# For the FastAPI migration, call `init_design_db()` from the application
# lifespan (or an Alembic migration) instead of relying on lazy creation.
#
# Previously: init_design_db()
