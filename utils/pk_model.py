...
Import-time dependency note (PK migration slice, 2026-07-31)
-----------------------------------------------------------
``import matplotlib.pyplot`` used to sit at module scope, which meant every
consumer of the *calculation* functions also had to load the plotting stack.
It now lives inside ``create_pk_plot()``, the only function that uses it, so a
headless service can import the two-compartment solver with NumPy alone.

This is an import-location change only. **No equation, constant, default,
integration scheme or return value was altered**, and the golden-vector baseline
for this module is unchanged.
"""
...
def create_pk_plot(
    ...
    # Imported here rather than at module scope so that the calculation
    # functions above stay importable without the plotting stack.
    import matplotlib.pyplot as plt
