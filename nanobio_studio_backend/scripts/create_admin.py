"""Create the first user account, safely.
...
* ``--role`` selects the account type. It defaults to ``admin`` so existing
  invocations are unchanged, but a clean installation needs at least one
  ``researcher``: an administrator manages access and is deliberately barred
  from authoring, reviewing or approving scientific records. The Validation
  Registry needs **two** researchers, because an experiment cannot be approved
...
#: A word followed by digits: the shape of nearly every default credential ever
#: shipped, and of the three this project had to retire.
#:
#: Expressed as a rule rather than a list on purpose. A list would have to
#: *contain* the retired passwords to reject them, which puts them back in the
#: source this cleanup removed them from — and it would only ever reject the
#: ones somebody remembered to add. The rule catches the whole family, including
#: the variants nobody thought of.
_WORD_THEN_DIGITS = re.compile(r"^[A-Za-z]+\d+$")

#: Bare dictionary words with no digits at all.
_COMMON_WORDS = frozenset({
    "password", "admin", "administrator", "nanobio", "letmein", "welcome",
    "secret", "qwerty", "login", "root", "test",
})
...
def _validate(password: str) -> None:
    ...
    if lowered in _COMMON_WORDS or _WORD_THEN_DIGITS.match(password):
        problems.append(
            "must not be a common default (a word followed by digits is the "
            "shape of nearly every shipped default password)")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create or reset a NanoBio Studio user account.")
    parser.add_argument("--username", required=True)
    parser.add_argument(
        "--role", default="admin",
        choices=[r.value for r in UserRole],
        help=("Account type. Defaults to admin. A clean installation "
              "needs at least one researcher, and the Validation "
              "Registry needs two."))
