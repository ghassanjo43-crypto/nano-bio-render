...
async def assign_project(run_id: int, project_id: Optional[int] = None,
    ...
    try:
        run = await svc.assign_run_to_project(
            session, owner_id=user.id, run_id=run_id,
            project_id=project_id, access=ctx)
    except svc.WorkspaceError as exc:
        return _error(exc.code, exc.message, status.HTTP_404_NOT_FOUND)
    return svc.run_to_detail(run)
...
async def compare_runs(
    ...
    for run_id in run_ids:
        try:
            run = await svc.get_run(session, owner_id=user.id,
                                    run_id=run_id, access=ctx)
        except svc.WorkspaceError as exc:
...
async def create_project(request: ProjectCreateRequest,
    ...
    project = await svc.create_project(
        session, access=ctx, owner_id=user.id, name=request.name,
...
ctx: AccessContext = Depends(get_access_context),
                        session: AsyncSession = Depends(get_auth_session)):
    rows = await svc.list_projects(session, owner_id=user.id, access=ctx)
    return {
        "projects": [
...
async def delete_project(project_id: int,
    ...
    if user.role is UserRole.VIEWER:
        return _error("forbidden", "Your role cannot delete projects.",
                      status.HTTP_403_FORBIDDEN)
    try:
        project = await svc.delete_project(session, access=ctx,
                                           owner_id=user.id,
                                           project_id=project_id)
    except svc.WorkspaceError as exc:
        return _error(exc.code, exc.message, status.HTTP_404_NOT_FOUND)
