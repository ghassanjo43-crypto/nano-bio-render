 ...
 */
import { organizationHeaders } from '../organizations/activeOrganization';
 ...
 */
/**
 * Exported so the Experimental Validation Registry client can reuse exactly
 * this transport: one 401 handler, one error shape, one place that decides a
 * response is trustworthy. A second fetch wrapper would drift from this one.
 */
export async function apiRequest<T>(
  ...
  // The active-organization header is merged here, in the one transport every
  // caller shares, rather than at each call site. The contract requires it on
  // *every* request including background polls; a per-call-site convention
  // would be one forgotten fetch away from a request answered across all of
  // the user's organizations while the interface showed one.
  const orgHeaders = organizationHeaders();
  try {
...
/* ==========================================================================
 * Scientific Readiness Framework
 * ========================================================================== */

/** GET /api/v1/science/studies/{id}/readiness */
export function getReadiness(studyId: number, signal?: AbortSignal) {
  return apiRequest<import('../pages/readiness/readinessTypes').ReadinessReport>(
    `/api/v1/science/studies/${studyId}/readiness`, { method: 'GET', signal },
    (body): body is import('../pages/readiness/readinessTypes').ReadinessReport =>
      typeof body === 'object' && body !== null && 'areas' in body,
  );
}

/** GET /api/v1/science/studies/{id}/records */
export function getScienceRecords(studyId: number, signal?: AbortSignal) {
  return apiRequest<
    import('../pages/readiness/readinessTypes').ScienceRecordListResponse>(
    `/api/v1/science/studies/${studyId}/records`, { method: 'GET', signal },
    (body): body is
      import('../pages/readiness/readinessTypes').ScienceRecordListResponse =>
...
/** POST /api/v1/science/studies/{id}/readiness/snapshots */
export function createReadinessSnapshot(
  studyId: number, trigger = 'manual', signal?: AbortSignal,
) {
  return apiRequest<{ id: number; created_at: string }>(
    `/api/v1/science/studies/${studyId}/readiness/snapshots`,
    { method: 'POST', body: JSON.stringify({ trigger }), signal },
    hasKeys<{ id: number; created_at: string }>('id', 'created_at'),
  );
}
