...
export default function WorkflowLayout() {
  ...
  // Targeting & Ligands is deliberately NOT in WORKFLOW_STEPS.
  //
  // That constant drives the gating rail, which is about data prerequisites:
  // each of its four steps needs the previous one's data to mean anything.
  // Targeting is optional — a study with no ligand is a legitimate passive
  // formulation — so it gates on the disease context only, and gets its
  // position from the pathway indicator instead.
  const isTargeting = location.pathname === '/workflow/targeting';
  if (isTargeting && !step1Complete) {
    return <Navigate to="/workflow/disease" replace />;
  }
