...
- **Hard-coded passwords in source:** `create_admin.py:31` and `set_admin_password.py:7` both
  contained a hard-coded provisioning password (redacted; now read from `NANOBIO_ADMIN_PASSWORD`); `db_init.py` seeded a one-word administrator password (redacted; now required from the environment). Both remediated.
...
| C5 | **Hard-coded provisioning password in source** (literal redacted; remediated) | `create_admin.py:31`, `set_admin_password.py:7` |
