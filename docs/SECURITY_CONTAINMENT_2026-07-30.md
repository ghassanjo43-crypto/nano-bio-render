...
| **Hard-coded provisioning password** (literal redacted) in `create_admin.py`, `set_admin_password.py` | Hard-coded in source. Not committed (no repo), but present in plaintext on disk | **YES if that password is used anywhere real** |
